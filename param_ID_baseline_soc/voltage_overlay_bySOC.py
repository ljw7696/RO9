# %% Per-SOC-region voltage OVERLAY grid (V(t); gray = region theta_eff,i, blue = V_data, red = combined)
import os
...
for ri, prof in enumerate(PROFILES_ORDER):
    ...
    for ci in range(ncol):
        ...
        for k in REG_CONDS[rg]:                       # region's theta_eff,i voltages (gray)
            Vs = CURVES.get((flabel(k), tk))
            if Vs is not None:
                ax.plot(t_ref, Vs, color="0.35", lw=0.9, alpha=0.6, zorder=1)
        ax.plot(t_ref, V_truth, color="tab:blue", lw=1.8, zorder=3)          # V_data
        Vc = COMB_CURVE.get(tk)
        if Vc is not None:
            ax.plot(t_ref, Vc, color="red", ls="--", lw=1.8, zorder=4)       # combined
        ax.set_title(f"{prof} s{soc}", fontsize=12)
        ax.set_ylim(V_truth.min() - 0.25, V_truth.max() + 0.10)
        ax.grid(alpha=0.3)
...
fig.legend(handles=handles, loc="upper right", fontsize=16, ncol=3, bbox_to_anchor=(0.99, 0.99))
fig.suptitle("Voltage: individual vs combined  (per SOC region)", y=1.0, fontsize=17)
plt.tight_layout(rect=[0, 0, 1, 0.97])
plt.savefig("meta_voltage_overlays_bySOC.png", dpi=120, bbox_inches="tight")
print("saved meta_voltage_overlays_bySOC.png")
