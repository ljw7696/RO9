...
# Saves results_bySOC_DLo.pkl with the SAME structure as results_bySOC.pkl.
import pickle
import numpy as np
...
for rg in REGIONS:
    for j, l in enumerate(LAB):
        ks = [k for k in OK if region(k[1]) == rg]        # ALL conditions (outliers included)
        n = len(ks)
        ...
        rec.update(mode="intercept-only(DL,outliers)", theta_comb=NOM[j] * 10**mu, ratio=10**mu,
...
print("Per-SOC-region combine (DL, OUTLIERS INCLUDED)\n")
for rg in REGIONS:
...
pickle.dump(results, open("results_bySOC_DLo.pkl", "wb"))
print("saved results_bySOC_DLo.pkl")
