...
# %% WLTP clean fit sweep: estimate theta on clean WLTP data at each SOC, save pkl
# CLEAN data (no discrepancy) -> inverse-crime test: can WLTP retrieve theta_nominal?
# For each SOC: build clean WLTP truth at theta_nominal + noise, run the multistart
# fit, save a pkl (wltp_clean_socXX_...), and print the median recovered ratio.
# WARNING: 4 full 30-start fits on the (long) WLTP profile -> this is slow.
import pickle

WLTP_PROFILE = "WLTP"
SOC_SWEEP = [0.30, 0.50, 0.65, 0.85]
N_JOBS = 12                              # parallel workers (self-contained here)
wltp_exp = make_experiment(profile_id=WLTP_PROFILE)

wltp_results = {}
for soc in SOC_SWEEP:
    log(f"================ WLTP clean fit at SOC {soc} ================")

    # clean truth on WLTP at this SOC (theta_nominal, no discrepancy) + noise
    base = make_base_params("Chen2020", soc=soc, sensitivity_ready=True)
    p_t, iv_t = prepare_sensitivity_inputs(base, fit_params, values=theta_values_nominal)
    sol_t = run_model(make_model("SPMe", options=MODEL_OPTIONS), p_t, wltp_exp, inputs=iv_t)
    ...
    # multistart fit on this clean WLTP data (worker uses profile_id + soc)
    results = Parallel(n_jobs=N_JOBS, verbose=10)(
        delayed(run_one_start)(
            sid, t_soc, V_soc, fit_params, PARAM_BOUNDS, log_scaled_params,
            soc, MODEL_OPTIONS, sigma_V_fit, MAX_NFEV, WLTP_PROFILE, JAC_MODE, DIFF_STEP)
        for sid in range(N_STARTS)
    )
    results.sort(key=lambda r: r["start_id"])
    wltp_results[soc] = results

    fname = f"wltp_clean_soc{int(soc * 100)}_{time.strftime('%Y%m%d_%H%M%S')}.pkl"
    with open(fname, "wb") as f:
        pickle.dump(results, f)
    log(f"  saved {fname}")

    # recovery check: median theta_hat/theta_nominal over survived (should be ~1)
    survived = [r for r in results if r["rmse_mV"] < 10.0]
    print(f"  recovery over {len(survived)}/{len(results)} survived "
          f"(median theta_hat/theta_nominal, ~1.0 = retrieved):")
